...
export async function GET() {
  const data = await prisma.joki.findMany({ orderBy: { createdAt: 'desc' } });
...
export async function POST(req: Request) {
  try {
    ...
    const res = await prisma.joki.create({
      data: { title, description, icon: icon || 'FaCode', features: featuresArray }
    });
